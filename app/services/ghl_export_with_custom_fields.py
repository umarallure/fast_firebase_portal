...
# HARDCODED MAPPING: Subaccount ID -> Custom Field ID (SSN)
# Enter your configuration here
SUBACCOUNT_CUSTOM_FIELD_MAPPING = {
    "2":"eLJZScAzeA9AfnAY12fK",
    "3":"C0tIW0JipyuiC8LR6ICK",
    "4":"0SWvSCYtMmhcFROmwQFO",
    "6":"yNabehfElfZrXHuzuX46",
    "9":"yCVipAYQgUxcnpBtRi7m",
    "11":"6Ce8Az6mvfOGFl71VnGV",
    "10":"fc6Fr2bAK026H3VtFBmy",
    "14":"KnfbTgoeQv91q5W3PGw0",
    "16":"goayeT8OIxICbDnXLQQd",
    "17":"93LuX73XPbzxbtbV9dSj",
    "18":"yi7tVMWCejGsa6ggSUhT",
    "19":"i4wRXGFd9QYAeuEIIL5i",
    "20":"2jefTXYWtbPsHsMQ9GKG",
    "54":"S2neFrAWPOCa8uE1xQrd",
    "53":"FcDxnjpHFMTxTYOzwLc3",
    "52":"jEuGDDPfLYhptiUF5hFO",
    "51":"9uIeWV4ugXhI7n4RTzzm",
    "50":"qE1A64eux5uwOdl3tKR6",
    "49":"ZqT6CquWgmEw4sYUFXxJ",
    "48":"z5Ks1w9bBqJrpCfqK4A0",
    "46":"eDqoxSp4TdhZeIPtDS5o",
    "44":"NzaXNqqhrgrkeu1Jfdwp",
    "43":"NKTPTnHdd6zVyoz5HNNM",
    "42":"112ub1DhpgYEp5XKdKsm",
    "41":"fhj9Rqky7uYantleNJjh",
    "40":"yJKqpyhCCAw3sSNebJZL",
    "39":"DIvu5pR6D9Pj9YVmFcWT",
    "38":"ryFTC0N5thCXB6sRoro7",    
    "37":"qKM1K8bRHs2HOeW86m2f",
    "36":"vNT5JHI9JQgKPBWNeofi",
    "35":"ebq0IMcVaVkRvK7KhBwm",
    "34":"SXRV0qthYsz0yRlfLzGY",
    "28":"0G2hAehjYuiqHREHvFMh",
    "27":"q1tsr3LrSa9Cenf9NR5a"
}

class GHLClientWithCustomFields:
...
async def process_export_request_with_custom_fields(export_request: ExportRequestWithCustomFields) -> bytes:
    """Process export request using the new GHL APIs and fetching custom fields"""
    try:
        logger.info(f"Starting new API export processing with custom fields for {len(export_request.selections)} selections")
        ...
        for selection in export_request.selections:
            ...
            # Get custom field ID from request OR fallback to hardcoded mapping
            custom_field_id = export_request.custom_field_mapping.get(account_id)
            if not custom_field_id:
                custom_field_id = SUBACCOUNT_CUSTOM_FIELD_MAPPING.get(account_id)
                if custom_field_id:
                    logger.info(f"Using hardcoded custom field ID {custom_field_id} for account {account_id}")
            ...
            client = GHLClientWithCustomFields(access_token, location_id)
            ...
            for pipeline_id in selection.pipelines:
                ...
                # Helper function to process a single opportunity
                async def process_single_opportunity(opp, semaphore):
                    async with semaphore:
                        ...
                        # Fetch contact details for custom field if mapping exists
                        ssn_value = ""
                        if custom_field_id:
                            contact_id = formatted_opp.get("Contact ID")
                            if contact_id:
                                contact_data = await client.get_contact(contact_id)
                                custom_fields = contact_data.get("customFields", [])
                                for cf in custom_fields:
                                    if cf.get("id") == custom_field_id:
                                        ssn_value = cf.get("value", "")
                                        break
                        
                        formatted_opp["SSN"] = ssn_value
                        return formatted_opp

                # Process opportunities concurrently with semaphore
                semaphore = asyncio.Semaphore(settings.max_concurrent_requests)
                tasks = [process_single_opportunity(opp, semaphore) for opp in opportunities]
        ...
        if all_opportunities:
            ...
            desired_columns = [
                ...
                "Days Since Last Updated", "Account Id", "SSN", "note1", "note2", "note3", "note4"
            ]
        ...
        logger.error(f"Error in process_export_request_with_custom_fields: {str(e)}")
        raise
